...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
  ...
  <div {{-- x-data="{ state: $wire.entangle('{{ $getStatePath() }}') }"  --}} x-data="{ isCollapsed: @js($isCollapsed()) }"
    ...
    {{ $attributes->merge($getExtraAttributes(), escape: false)->class(['bg-white relative dark:bg-gray-800 dark:border-gray-600']) }}>
    ...
    <div class="px-4{{ $isAddable ? '' : ' py-2' }} text-[12px]">

      <table class="it-table-repeater w-full table-fixed border-collapse" x-show="! isCollapsed">
